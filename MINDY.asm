...
;
;       by John Hardy and contains code from the MINT project by Ken Boak and Craig Jones. 
;
...
; *****************************************************************************
        ...
        TRUE        EQU 1		; not FF, for Mindy
        FALSE       EQU 0
        EMPTY       EQU 0		; for an empty macro, ctrl-<something>=macro, ie ctrl-h = backspace macros (in Mindy)

        MindyDataSize      EQU 26*2*2	; A..Z, a..z words
...
; ***********************************************************************
; Initial values for user MindyVars		
; ***********************************************************************		
...
iOpcodes:
        ...
        DB     lsb(aNop_)       ;dd     \]
        DB     lsb(go_)         ;de     \^  ( -- ? ) execute Mindy definition a is address of Mindy code
        ...
        LITDAT 5
        DB    lsb(rpop_)        ;       { ( -- n ) pop from Mindy return stack 
        DB    lsb(aNop_)        ;                  
...
init:                           ;=68
        ...
        LD HL,MindyData          ; init namespaces to 0 using LDIR
        LD DE,HL
        INC DE
        LD (HL),0
        LD BC,MindyDataSize
...
printdec:                           ;=36
        LD DE,-10000			; Mindy ., 5th location of a dev number
...
utilTable:
        ...
        DB lsb(prompt_)     ;\#5    ( -- )        print Mindy prompt 
